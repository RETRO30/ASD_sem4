"""Лабораторная работа 2: поиск вложенных треугольников на плоскости.

Теория:
- Треугольник однозначно задается тремя неколлинеарными точками.
- Из `n` точек можно построить `C(n, 3) = n! / (3!(n - 3)!)` треугольников.
- Один треугольник считается вложенным в другой, если все его вершины
  принадлежат внешнему треугольнику (включая границу).
"""
...
def generate_triangles(points: list[Point]) -> list[Triangle]:
    """Генерирует все уникальные тройки точек как треугольники.

    Теоретическая основа: полный перебор сочетаний без повторений по 3 элемента.
    Индексы выбираются так, что `i < j < k`, поэтому каждый набор вершин
